...
def main():
    ...
    parser.add_argument("--model-path", type=str, default="models/best_model.pt",
                       help="Path to model checkpoint (e.g., models/best_model.pt)")
    ...
    print(f"  Hidden dim: {ckpt_args.get('hidden_dim', 'unknown')}")
    print(f"  Num layers: {ckpt_args.get('num_layers', 'unknown')}")
    print(f"  Dropout: {ckpt_args.get('dropout', 'unknown')}")
    print(f"  Use attention: {ckpt_args.get('use_attention', 'unknown')}")
    print(f"  Loss function: {ckpt_args.get('loss', 'unknown')}")
    ...
    val_data = create_shock_training_data(
        val_years, 
        args.embeddings_dir, 
        ckpt_args.get('shock_threshold', 0.15),
        balanced_sampling=False
    )

    # Reconstruct model - infer dimensions from data
    ...
    # Reconstruct model with architecture from checkpoint
    model = ShockPropagationGNN(
        node_in_dim=node_dim,
        edge_in_dim=edge_dim,
        hidden_dim=ckpt_args.get('hidden_dim', 64),  # Fallback to reasonable default
    ...
    with torch.no_grad():
        for metadata in val_data:
            example = load_and_process_example(metadata, 'cpu', 
